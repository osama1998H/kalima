frappe.pages['courses-students-res'].on_page_load = function (wrapper) {
    var page = frappe.ui.make_app_page({
        parent: wrapper,
        title: __('Courses Students Result Sheet'),
        single_column: true
    });
    var crv = 0;
    ...
    function submit_results() {
        ...
        $(wrapper).find('.student-table-container tbody tr').each(function () {
            let student_data = {
                ...
                final_exam_result: $(this).find('td:eq(2) input').val(),
                curve: $(this).find('td:eq(3) input').val(),
                present: $(this).find('td:eq(4) input').val(),
                result: $(this).find('td:eq(5) input').val(),
                status: $(this).find('td:eq(6) input').val(),
