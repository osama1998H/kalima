...
frappe.ui.form.on("Student", {
    refresh(frm) {
        ...
        {   // Function to check if a document already exists
            function document_exists(doc_name) {
                return frm.doc.student_documents.some(function(row) {
                    return row.document === doc_name;
                });
            }
            ...
            // Add documents if they do not already exist
            documents_to_add.forEach(function(doc_name) {
                if (!document_exists(doc_name)) {
                    let new_row = frm.add_child('student_documents');
                    frappe.model.set_value(new_row.doctype, new_row.name, 'document', doc_name);
                }
            });
        ...
        if(true)
            {   // Function to check if a document already exists
                ...
                documents_to_add.forEach(function(doc_name) {
                    if (!document_exists(doc_name)) {
                        let new_row = frm.add_child('optional_student_documents');
                        frappe.model.set_value(new_row.doctype, new_row.name, 'document', doc_name);
                    }
                });
    
                // Refresh the field to update the UI
                frm.refresh_field('optional_student_documents');
            }
            frm.set_df_property('student_documents', 'cannot_add_rows', true);
            frm.set_df_property('optional_student_documents', 'cannot_add_rows', true);
